...
class ChatEDA:  # changed class name from ChatEDAWithPlotly to ChatEDA
    ...
    def prepare_data(self, df):
        ...
        df['is_media'] = df['message'].str.contains('', case=False, na=False)
    ...
    def create_3d_activity_visualization(self):
        ...
        fig = go.Figure(data=[go.Scatter3d(
            x=activity['weekday'], y=activity['hour'], z=activity['message_count'],
            mode='markers',
            marker=dict(
                size=8,
                ...
                showscale=True,
                colorbar=dict(
                    title=dict(text='Messages', font=dict(size=12, color='black')),
                    tickfont=dict(size=10, color='black'),
                    thickness=20, len=0.7, x=1.02, xanchor='left'
                )
            ),
    ...
    def create_heatmap_with_colorbar(self):
        """ Create heatmap of hourly activity by day """
        ...
        fig = go.Figure(data=go.Heatmap(
            ...
            colorscale='YlOrRd',
            colorbar=dict(
                title=dict(text='Messages', font=dict(size=12)),
    ...
    def create_response_time_3d(self):
        """ Create 3D visualization of response times """
        ...
        fig = go.Figure(data=[go.Scatter3d(
            x=resp_df['weekday'], y=resp_df['hour'], z=resp_df['response_time'],
            mode='markers',
            marker=dict(
                size=6,
                ...
                showscale=True,
                colorbar=dict(
                    title=dict(text='Response Time (min)', font=dict(size=11)),
    ...
    def create_comprehensive_dashboard(self):
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Daily Message Volume','Time Period Distribution','Sender Activity','Weekend vs Weekday'),
            specs=[[{'type':'scatter'},{'type':'bar'}],
                   [{'type':'bar'},{'type':'bar'}]]
        )
        daily_counts = self.df.groupby('date').size().reset_index(name='count')
        fig.add_trace(
            go.Scatter(x=daily_counts['date'], y=daily_counts['count'], mode='lines+markers', name='Daily Messages'),
            row=1, col=1
        )
